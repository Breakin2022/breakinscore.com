...
class ApiControllerV2 extends Controller
{
  ...
  public function insertScorefromadmin(Request $request){
    $currentTime = date('Y-m-d H:i:s');
    $judgeId        = $request->judgeId;
    $participantId  = $request->participantId;
    $competitionId  = $request->competitionId;
    ...
    $criterias = DB::table('competition_criterias')->where('competitionid', $competitionId)->join('criterias', 'criterias.id' ,'=', 'competition_criterias.criteriaid')->get();

    $co = 0;
    foreach ($score as $obj) {
      $obj = json_decode($obj);
      $scoreArray[] = [
        'score'  => $obj,
        'id'        => $criterias[$co]->id,
  ...
  public function insertscoresofmatch($matchId,$competitionid)
  {
    ...
    $arryColletion['judgeId'] = $judgeId = Auth::id();
    $time = date('Ymdhis');
    $date = date("Y-m-d");

    $arryColletion['competition'] = $competition = DB::table('competitionVenues')->where('id', $competitionid)->whereDate('start_date', ">=", $date)->first();
    $arryColletion['criterias'] = $criterias = DB::table('competition_criterias')->where('competitionid', $competitionid)->join('criterias', 'criterias.id' ,'=', 'competition_criterias.criteriaid')->get();
    $arryColletion['match'] = $match = DB::table('matches')->select(DB::Raw('
    matches.*,
    (Select teams.name as firstTeamName from teams where teams.id = matches.firstTeam) as firstTeamName,
    (Select teams.name as firstTeamName from teams where teams.id = matches.secondTeam) as secondTeamName
    '))
    ->where('id','=',$matchId)->where('competitionId','=',$competitionid)->orderBy('id')->first();
    $arryColletion['teamOneParticipants'] = DB::table('participant')->leftjoin('teamsmembers','participant.id','=','teamsmembers.pid')->where('teamsmembers.tid', $match->firstTeam)->get();
    $arryColletion['teamTwoParticipants'] = DB::table('participant')->leftjoin('teamsmembers','participant.id','=','teamsmembers.pid')->where('teamsmembers.tid', $match->secondTeam)->get();

    return view('livecompetition.matchScoring')->with('arryColletion', $arryColletion);
  }
  ...
  public function CompetitionList(Request $request){
    ...
    {
      return view('livecompetition.list')->with('arryColletion', $arryColletion);
    }
  }
