...
class ReportControllerOld extends Controller
{
  ...
  public function show(Request $request,$id){
   ...
   // 'teamtwo member one id' => $match->teamTwo->members[0]->pid,
   //
   // 'teamtwo member two id' =>   $match->teamTwo->members[1]->pid,
   //  ]
   //  );
    if ($match == null) {
    ...
    if ($obj->type == 1) {
      ...
      for ($i=1; $i <= $obj->totalRounds; $i++) {
        ...
        $p1Data = $participant1Data->where('roundNo',$obj->currentRound);
        $p2Data = $participant2Data->where('roundNo',$obj->currentRound);
    ...
    if ($obj->type == 2) {
      ...
      for ($i=1; $i <= $obj->totalRounds; $i++) {
        ...
        $p1Data = $participant1Data->where('roundNo',$obj->currentRound);
        $p2Data = $participant2Data->where('roundNo',$obj->currentRound);
        $p3Data = $participant3Data->where('roundNo',$obj->currentRound);
        $p4Data = $participant4Data->where('roundNo',$obj->currentRound);
